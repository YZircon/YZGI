...
// 把这玩意关了, 使得 OBJ Loader 不会往 console 输出东西
        ...
        for(int i = 0; i < loader.LoadedMeshes.size(); i++){
            ...
            if(_m == nullptr){
                if(materials.find(loader.LoadedMeshes[i].MeshMaterial.name) != materials.end()){
                    ...
                    meshMat = new Material(DIFFUSE,Eigen::Vector3f(0,0,0)); // 这个地方应该是个错误材质, 就是既没有指定材质模型又没有附带材质的时候显示的东西
                }
            }else{
            ...
            for(int j = 0; j < loader.LoadedMeshes[i].Indices.size(); j += 3){
                ...
                Triangle* tri = new Triangle(v0, v1, v2, meshMat); // 一个潜在的问题是如果不同的Mesh有共用三角形且材质相同会导致实例化同一个三角形多次(但扬掉也不合适吧? 扬掉就不是不同mesh了)
